...
def parse_human_eval_jsonl():
    with open('miniset.jsonl', 'r') as f:
...
def create_experiment_folders():
    ...
    for i in range(TEST_COUNT):
        ...
        with open('inputs_' + str(i) + '.json', 'w', encoding='utf-8') as f:
            inputs_obj = {
                "base_input": json_obj["base_input"],
                "plus_input": json_obj["plus_input"],
                "atol": json_obj["atol"],
                "contract": json_obj["contract"],
                "entry_point": json_obj["entry_point"]
            }
            json.dump(inputs_obj, f, indent=2)

            print("Inputs file for " + str(i) + " created")
...
def create_csv():
    ...
    with open("results.csv", "w", encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        header = ["ID", "Correctness", "Validity",
                 'security_rating', 'bugs', 'code_smells',
